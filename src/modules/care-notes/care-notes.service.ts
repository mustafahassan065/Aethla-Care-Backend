...
export class CareNotesService {
  ...
  async create(dto: any) {
    ...
    const doc: any = {
      ...
      notes:        dto.notes        || '',
      vitals:       dto.vitals       || '',
      familyShared: dto.familyShared === true || dto.familyShared === 'true',
      createdBy:    dto.createdBy,
    }

    if (dto.clientId && dto.clientId !== '') {
      try { doc.clientId = new Types.ObjectId(dto.clientId) } catch {}
    }
    if (dto.caregiverId && dto.caregiverId !== '') {
      try { doc.caregiverId = new Types.ObjectId(dto.caregiverId) } catch {}
    }
    if (dto.scheduleId && dto.scheduleId !== '') {
      try { doc.scheduleId = new Types.ObjectId(dto.scheduleId) } catch {}
    }
